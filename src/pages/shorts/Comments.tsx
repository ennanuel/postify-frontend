...
const Comments = () => {
    ...
    const { comments, showForMobile, commentId, commentInFocus, updateCommentId, handlePostComment, handleLikeComment, hideAll } = useContext(PostContext);
    const showFull = useMemo(() => showForMobile === 'comment', [showForMobile]);
    ...
    return (
        <div className={`post-comments absolute top-0 left-0 w-full h-full z-[4] lg:relative grid grid-rows-[auto,auto,1fr,auto] overflow-y-scroll bg-black-500 transition-[transform,opacity] origin-bottom ${showFull ? 'row-span-full' : 'scale-y-75 lg:scale-y-100 opacity-0 lg:opacity-100 pointer-events-none lg:pointer-events-auto'}`}>
            <div className="flex items-center justify-end sticky top-0 py-2 px-4 border-y">
                {
                    showFull &&
                    <button className="w-8 flex items-center justify-center" onClick={hide}>
